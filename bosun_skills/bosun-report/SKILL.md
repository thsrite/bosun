...
若当前任务由父任务派生，完成、失败或需要父任务答复时必须立即回报，不能只在本终端输出后等待。父任务通过 `/spawn` 或 `/reply` 的监控响应接收回报；`needs_input` 会保留会话供父任务继续回复，最终回报会触发会话回收。

父任务指定了完整结果文件时，必须先保存结果和验证证据再回报，确保父任务读取时文件已经存在；不要把完整结果只留在回报之后的终端正文里。回报必须使用当前子任务自己的 `BOSUN_TASK_ID` 和 `BOSUN_TASK_TOKEN`，不得代替父任务回报，也不得自行派生下一层子任务。
