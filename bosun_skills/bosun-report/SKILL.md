...
收尾固定两步，顺序不能反：**①先运行 report.sh 回报状态 → ②回报之后把完整结论正文
作为你最后一条消息打印到终端**（report 之后不得再有工具调用）。用户只看最后那条正文：
写在工具调用之前会被折叠进命令记录，塞进 `--summary` 参数用户要点开命令才看得到，
只说「见上」等于没写。`summary` 是给工作台看的一句话摘要（≤200 字），替代不了正文。

第一步运行本 skill 目录下的 `scripts/report.sh`，二选一传状态：
...
第二步：回报成功后，紧接着把本轮完整的结论/分析/待拍板问题正文，作为你最后一条
消息完整打出来，然后停下。这条正文才是用户实际读到的收尾内容。
