...
手机端终端固定显示两排快捷键：第一排 Esc、Tab、Ctrl-C、↑、退格、文件上传，第二排键盘、Space、←、↓、→、Enter，方向键按倒 T 排列。点击键盘图标唤起输入法；操作快捷键不会转移终端输入焦点。终端使用系统等宽字体，窄屏触控端保持 12px 字号和紧凑字距。

终端复制：手机端长按并拖动选区，松手后点击「复制选中文本」；桌面端在 Claude 等启用鼠标跟踪的终端里，普通左键拖选松手即复制，直接读取浏览器终端的字符缓冲区，不依赖 CLI 的自动复制文本。普通点击及带修饰键的手势仍沿用原行为。复制会合并终端软换行，以及能根据右边界和缩进识别的正文排版续行；空行、独立列表项、代码块和表格保留换行，无法确定的换行不强行删除。
